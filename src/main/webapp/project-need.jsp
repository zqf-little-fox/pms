...
<script type="text/javascript">
    function batchDelete() {
        if (confirm("你确定要删除吗？")) {
            ...
            $.ajax({
                type:"post",
                url:"${pageContext.request.contextPath}/anal/del",
                data:{"_method": "delete","ids":ids},
                success: function (msg){
                    if (msg.statusCode == 200){
                        alert("删除成功");
                        window.location.href = "${pageContext.request.contextPath}/anal/list";
                    }
                },
                error: function () {
                    alert("删除失败");
...
<c:forEach items="${list}" var="need" varStatus="index">
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	<td><input name="id" type="checkbox" id="id" value="${need.id}" class="np"></td>
	<td>${index.count}</td>
	<td>${need.title}</td>
	<td align="center"><a href=''><u>${need.proname}</u></a></td>
	<td><fmt:formatDate value="${need.addtime}" pattern="yyyy-MM-dd"/> </td>
	<td><fmt:formatDate value="${need.updatetime}" pattern="yyyy-MM-dd"/></td>
	<td><a href="${pageContext.request.contextPath}/anal/edit/${need.id}">编辑</a> | <a href="${pageContext.request.contextPath}/anal/detail/${need.id}">查看详情</a></td>
</tr>
</c:forEach>
<tr bgcolor="#FAFAF1">
